void playing1Page() {
  ...
  // detect elasiticity change
  fsrReading = arduino.analogRead(fsrAnalogPin);
  YtransitionDelta = float(fsrReading) / 2046;
  XtransitionDelta = YtransitionDelta/25;
  Xtransition = XtransitionDelta + XtransitionInitial;
  Ytransition = YtransitionDelta + YtransitionInitial;
  LEDbrightness = int(map(fsrReading, 0, 1023, 0, 255));
  arduino.analogWrite(11, LEDbrightness);
