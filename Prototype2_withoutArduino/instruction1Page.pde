void instruction1Page() {
  ...
  text("   collect the coins", -17*a, -10*a);
  text(" - Select the target number of coins. You win the game once they are", -17*a, -8*a);
  text("   successfully collected:", -17*a, -7*a);
  
  text(" - Choose a planet:", -17*a, -3*a);
  
  // coins
  image(photoCoin, -1.5*a, -5.25*a);
  
  // + - buttons
  // + button
  if (mouseX>22*a && mouseX<23*a && mouseY>14.25*a && mouseY<15.25*a){
    fill(buttonPMNew);
    if (mousePress){
      mousePress = false;
      soundButton.play();
  ...
  rect(2.5*a, -5.25*a, a, a, 5, 5, 5, 5);
  // - button
  if (mouseX>23.5*a && mouseX<24.5*a && mouseY>14.25*a && mouseY<15.25*a){
    fill(buttonPMNew);
    if (mousePress){
      mousePress = false;
      soundButton.play();
  ...
  rect(4*a, -5.25*a, a, a, 5, 5, 5, 5);
  // +- icons
  fill(0);
  text("+", 2.25*a, -5*a);
  text("-", 3.75*a, -5*a);
  fill(titleColor);
  text(targetCoin, 0*a, -5*a);
  ...
  textSize(18);
  fill(255);
  text("Avoid bomb", -15*a, 4.5*a);
  image(photoBomb, -8*a, 4*a);
  text(", whose explosion stains the screen; Avoid vortex", -7*a, 4.5*a);
  image(photoVortexEG, 16*a, 4*a);
  
  // life
  fill(lifeColor);
  text("# of lives:", -15*a, 7.5*a);
  image(photoLife, -9*a, 7*a);
  image(photoLife, -7*a, 7*a);
  image(photoLife, -5*a, 7*a);
  
  // elasticity
  fill(elasticityColor);
  text("Elasticity of trampoline: 0.50", -15*a, 10*a);
  //text("0.260", -10.5*a, 10*a);
